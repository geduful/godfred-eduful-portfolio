...
- **Automatic project publishing** — push any public repo to GitHub and it appears in "More from GitHub" on the next visit; tag a repo with the `portfolio` topic and it becomes a full project card (with a "New" badge when freshly pushed). Deleting a repo (or making it private) removes it the same way — auto cards and feed rows vanish with the next fresh sync, and curated cards whose linked repo is confirmed gone are hidden too. A daily GitHub Action (`.github/workflows/sync-github.yml`) refreshes the committed snapshot so SEO and first-paint data stay current too.
